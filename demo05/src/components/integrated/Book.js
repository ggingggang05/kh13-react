...
const Book = () => {
    ...
    const listener = useCallback(throttle((e)=>{
        const scrollableHeight = document.documentElement.scrollHeight - window.innerHeight;// 문서의 전체 높이
        const scrollTop = window.scrollY;// 현재 스크롤 위치
        const scrollPercent = (scrollTop / scrollableHeight) * 100;// 스크롤 위치를 퍼센트로 계산
